...
class ParsnipModel(nn.Module):
    ...
    def _compute_amplitude_spec(self, spec_weight, model_spectra, obs_spectra):
        """Takes in observed spectra, model spectra, and weights
            Returns amplitude mean, and logvar, used to multiply spectra and scale it"""
        num = torch.sum(spec_weight * model_spectra * obs_spectra)
        denom = torch.sum(spec_weight * model_spectra * model_spectra)

        # With augmentation, can very rarely end up with no light curve points. Handle
        # that gracefully by setting the amplitude to 0 with a very large uncertainty.
        denom[denom == 0.] = 1e-5

        amplitude_mu_spec = num /denom 
        amplitude_logvar_spec = torch.log(1. / denom)

        return amplitude_mu_spec, amplitude_logvar_spec
    ...
    def loss_function(self, result, return_components=False, return_individual=False):
        ...
        # spectra_nll_update = 0.5* obs_weight * (obs_spectra - model_spectra)^2
        # Add both nlls
        ...
        #print(f'loss values are: nll={nll}, kld={kld}, penalty={penalty}, amp_prob={amp_prob}, redshift_nll={redshift_nll}')
