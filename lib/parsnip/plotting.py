...
def plot_light_curve(light_curve, model=None, count=100, show_uncertainty_bands=True,
                     ...
                     sncosmo_model=None, sncosmo_label='SNCosmo Model', ax=None, name=None):
    """Plot a light curve
    ...
    if reference_time != 0.:
        ax.set_xlabel(f'Relative Time (days + {reference_time[0]:.2f})')
    else:
    ...
    if normalize_flux:
	    ...
	    ax.set_ylabel(f'Flux ($ZP_{{AB}}$=25.0)')
...
def plot_confusion_matrix(predictions, classifications, figsize=(5, 4), title=None,
    ...
    ax.set_xticklabels(type_names, rotation=60, ha='right')
