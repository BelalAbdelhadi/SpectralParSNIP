...
class ParsnipModel(nn.Module):
    ...
    def loss_function(self, result, return_components=False, return_individual=False):
        ...
        # Regularization of spectra (w.r.t phase)
        second_der = (
            result['model_spectra'][:, :, 2:] + result['model_spectra'][:,:,:-2]
            - 2 * result['model_spectra'][:,:,1:-1]
        )

        phase_penalty = self.settings['phase_penalty'] * second_der**2
        ...
        if return_individual:
            ...
            phase_penalty = torch.sum(torch.sum(phase_penalty, axis=2), axis=1)
        else:
            nll = torch.sum(nll)
            kld = torch.sum(kld)
            penalty = torch.sum(penalty)
            phase_penalty = torch.sum(phase_penalty)
            amp_prob = torch.sum(amp_prob)
            redshift_nll = torch.sum(redshift_nll)

        if return_components:
            return torch.stack([nll, kld, penalty, phase_penalty, amp_prob, redshift_nll])
        else:
            return nll + kld + penalty + amp_prob + redshift_nll + phase_penalty

    def score(self, dataset, rounds=1, return_components=True, sample=True):
